...
*  Where is GLib from and why does it exist?  Who uses it, and for
  ...
  *Response:* GLib's code was separated from GTK+. It provides advanced data structures, string utilities, etc. Probably a less confusing name?

*  What are the most important pros and cons of using GLib, as opposed
  to another library, or just using core C features?

  *Response:* GLib is cross-platform and supports desktop programming. But it does still not support network stuff.

*  What do you have to do to compile and run a "Hello GLib" example?

   *Response:* Using flags given by `FLAGS=$(shell  pkg-config --cflags --libs glib-2.0)`.
